...
//    float3 mainLightColor = tmpColor.xyz;
    ...
    float3 defaultLightColor = saturate(max(float3(0.05, 0.05, 0.05) * _Unlit_Intensity, max(
        SampleBakedGI(posInput.positionWS, float3(0.0, 0.0, 0.0), input.texCoord1.xy, input.texCoord2.xy, true),
        SampleBakedGI(posInput.positionWS, float3(0.0, -1.0, 0.0), input.texCoord1.xy, input.texCoord2.xy, true)
        ) * _Unlit_Intensity));
    ...
    float3 envLightColor = saturate(SampleBakedGI(posInput.positionWS, utsData.normalDirection, input.texCoord1.xy, input.texCoord2.xy, true));
